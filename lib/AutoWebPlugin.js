...
class AutoWebPlugin {
	 ...
	 *      get WebPlugin template's file path
	 ...
	 *
	 * options.templateContent {string,function}
	...
	constructor(pageDir, options) {
		...
		this.entryMap = {};
		this.webpackEntry = {};
		const { template, templateContent, entry, filename, ignorePages, preEntrys, postEntrys, } = options;
		...
		pageNames.forEach(pageName => {
			...
			if (typeof templateContent === 'string') {
				templateHtmlString = templateContent;
			} else if (typeof templateContent === 'function') {
				templateHtmlString = templateContent(pageName);
	...
	apply(compiler) {
		...
		Object.keys(entryMap).forEach(entryName => {
			...
			const { templatePath, templateHtmlString } = entryMap[entryName];
			new WebPlugin({
				template: templatePath,
				templateContent: templateHtmlString,

...
class AutoWebPlugin {
	...
	apply(compiler) {
		...
		const { entryMap } = this;
		const { outputPagemap, ...otherOptions } = this.options;
		...
		compiler.hooks.emit.tap('AutoWebPlugin', (compilation) => {
			if (outputPagemap) {
				...
				Object.keys(this.entryMap).forEach((name) => {
					outJson[name] = util.getDependenciesFilesForChunks([name], compilation);
					outJson[name].html = `${this.entryMap[name].htmlOutputFilename}.html`;
				});
				util.outPagemap(outJson, outputPagemap, compilation);
			}
		});
	}

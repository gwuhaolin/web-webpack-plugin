...
class Resource {
	...
	constructor(node, options) {
		...
		if (nodeName === 'script') {
			...
			} else if (rel === 'import' && typeof href === 'string') {
				this.type = 'fragment';
				this.parseQuery(href);
			}
		...
		const outToNewNodes = (fileName, fileContent) => {
			...
			if (type === 'script') {
				...
				if (fileName.endsWith('.html')) {
					if (resourceAttr.inline) {
						const nodes = parse5.parseFragment(fileContent);

						newNodes = newNodes.concat(nodes.childNodes);
					} else {
					...
					if (
						...
						try {
							...
							const fileContent = fs.readFileSync(filePath, {
								...
								encoding: ['script', 'style', 'fragment'].indexOf(this.type) >= 0 ? 'utf8' : undefined
							});

	...
	out(compilation, publicPath) {
		...
		const outToNewNodes = (fileName, fileContent) => {
			if (type === 'script') {
				...
				this.node.attrs[this.otherAttrIndex].value = url.resolve(publicPath || '', fileName);
				...
				if (compilation.chunks.findIndex(chunk => chunk.name === this.chunkName) > -1) {
					...
					if (this.chunkName) {
						...
						try {
							...
							const distDirPath = compilation.outputOptions.path;
							// 确保输出目录存在
							if (!fs.existsSync(distDirPath)) {
								fs.mkdirSync(distDirPath);
							}
							// 复制原文件到webpack配置的输出目录
							fs.createReadStream(filePath).pipe(fs.createWriteStream(path.resolve(distDirPath, filename)));

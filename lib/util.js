...
function getDependenciesFilesForChunks(chunks, compilation) {
	const outJson = {
	...
	chunks.forEach((name) => {
		...
		Array.from(chunkGroup._children).forEach((em) => {
			em.chunks.forEach((chunk) => {
				if(chunkGroup.chunks.includes(chunk)){
		...
		outJson.asyncJs = (asyncFiles
			.filter((em) => em.endsWith('.js')));
		outJson.asyncCss = (asyncFiles
			.filter((em) => em.endsWith('.css')));
	});
	return outJson;
}

function outPagemap(outJson, outputPagemap, compilation) {

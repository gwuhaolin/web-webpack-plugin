...
class HTMLDocument {
    ...
    constructor(htmlFilePath) {
        ...
        if (HTMLDocumentCacheMap.hasOwnProperty(htmlFilePath)) {
            return HTMLDocumentCacheMap[htmlFilePath].clone();
        } else {
            if (typeof htmlFilePath === 'string') {
                htmlFilePath = path.resolve('.', htmlFilePath);
                htmlString = fs.readFileSync(htmlFilePath, {
                    encoding: 'utf8'
                });
            }
            this.document = parse5.parse(htmlString);
            this._findOutAll();
            HTMLDocumentCacheMap[htmlFilePath] = this.clone();
        }
    }
    ...
    _findScriptStyleTagComment(node) {
        node.childNodes.forEach(childNode => {
            const resource = new Resource(childNode);
            const { type } = resource;
            if (type === 'script') {

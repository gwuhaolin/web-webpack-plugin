...
class WebPlugin {
	...
	apply(compiler) {
				...
				if (typeof options.template === 'string') {
					const notHas =
						Array.from(compilation.fileDependencies).findIndex(
							(filePath) => filePath === options.template
						) === -1;
					if (notHas) {
						compilation.fileDependencies.add(options.template);
					}
				}
			});
		}
		...
		compiler.hooks.emit.tap('WebPlugin', (compilation) => {
			...
			// out fragments to html document
			this.htmlDocument.fragmentResources.forEach(fragmentResource => {
				fragmentResource.out(compilation);
			});
